...
const FooterLink = ({ to, children }: { to: string; children: React.ReactNode }) => {
  return (
    <Link
      to={to}
      className='border-transparent border-b-1 font-["Monoton"] hover:border-slate-900 hover:border-b-1 [&.active]:border-slate-900 [&.active]:border-b-2 [&.active]:font-bold [&.active]:text-slate-800'>
      {children}
    </Link>
  )
}
...
export const Route = createRootRoute({
  component: () => {
    ...
    return (
      <div className='flex min-h-screen min-w-[650px] flex-col text-slate-600'>
        <header className='relative flex flex-row items-center justify-between p-6'>
          <Link to='/' className=' border-transparent border-b-3 font-bold text-5xl hover:border-slate-900 hover:border-b-3 hover:text-slate-800 [&.active]:text-slate-800'>
            <h1>Yuriy Yarosh</h1>
          </Link>
        ...
        <footer className='mt-auto flex min-w-[700px] flex-row items-center justify-between p-4 text-sm'>
          <p className='p-2 font-extralight text-[10px]'>
