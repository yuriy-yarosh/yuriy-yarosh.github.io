...
interface SunDialResult {
  ...
  sunset: Date
  solarNoon: Date
  progress: number // 0-90, progress through the day
}

// Timezone coordinates as array for functional operations
...
// Pure function to get coordinates from timezone
const getCoordinatesFromTimezone = (timezoneOffset: number) => {
  ...
  // Find closest match by calculating distance for each entry
...
// Pure function to calculate sun times using astronomical formulas
...
// Pure function to determine time of day
...
const calculateSunDialResult = (now: Date): SunDialResult => {
  ...
  const progress = Math.min(90, Math.ceil(rawProgress * 9) * 10) // Ceil to nearest 10% increment as 0-90
