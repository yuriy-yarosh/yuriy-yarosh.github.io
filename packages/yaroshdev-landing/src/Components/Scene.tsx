...
import { Color, type Group, MathUtils, type Mesh, type Object3D, Vector3, Fog } from 'three'
...
export const Scene = React.memo(() => {
  ...
  const shouldVignette: boolean = ['evening', 'night', 'dusk', 'dawn', 'morning'].includes(timeOfDay)
  const hasStars: boolean = ['evening', 'night', 'dusk', 'dawn'].includes(timeOfDay)
  const sunny: boolean = ['noon', 'afternoon'].includes(timeOfDay)
  ...
  return (
    <Canvas camera={{ position: [0, 0, 2.5], fov: 60 }} fallback={<div>Sorry no WebGL supported!</div>}>
      ...
      <EffectComposer>
        ...
        {sunny ? <Bloom luminanceThreshold={0.3} luminanceSmoothing={3} height={720} /> : <></>}
        {isDark ? (
      ...
      <Stars radius={1} depth={5} count={hasStars ? 2000 : 0} factor={timeOfDay === 'night' ? 0.4 : 0.2} saturation={timeOfDay === 'night' ? 0.3 : 0.1} fade />
