...
export const Scene = React.memo(() => {
  ...
  return (
    <Canvas camera={{ fov: 60, position: [0, 0, 2.5] }} fallback={<div>Sorry no WebGL supported!</div>} gl={{ antialias: true, powerPreference: 'low-power' }}>
      <PerformanceMonitor
        onDecline={() => {
      ...
      <EffectComposer>
        ...
        {shouldBloom ? (
          <Fragment key='bloom'>
            <Bloom height={720} luminanceSmoothing={3} luminanceThreshold={0.3} />
          </Fragment>
        ) : (
          <Fragment key='no-bloom' />
        )}
        {isDark ? (
          <DepthOfField bokehScale={isAnimating ? 2 : 9} focalLength={1} focusDistance={0.5} height={720} />
        ) : (
          <DepthOfField bokehScale={isAnimating ? 1 : 4} focalLength={0.5} focusDistance={0.3} height={720} />
        )}
      </EffectComposer>

      {hasStars && (
        <Stars count={hasStars ? numberOfStars : 0} depth={5} factor={timeOfDay === 'night' ? 0.4 : 0.2} fade radius={1} saturation={timeOfDay === 'night' ? 0.3 : 0.1} />
      )}
