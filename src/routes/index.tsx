...
const HomeLink = ({ title, to }: { title: string; to: string }) => {
	...
	return (
		...
		>
			<h3 className="text-nowrap font-title text-md text-neutral md:text-lg lg:text-2xl xl:text-3xl 2xl:text-4xl">
				{title}
			</h3>
		</Link>
	);
	...
	return (
		...
		>
			<nav className="flex flex-row justify-center gap-6 p-4 max-[360px]:gap-3 md:gap-5 md:p-6 lg:flex-col lg:justify-start lg:p-8 xl:p-12">
				<HomeLink title={m.blog_page()} to="/blog" />
				<HomeLink title={m.projects_page()} to="/projects" />
				<HomeLink title={m.hire_page()} to="/hire" />
			</nav>
		</CatchBoundary>
	);
};
