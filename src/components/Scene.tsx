...
import { useAnimationState, useColors, useResponsiveGridSize } from "#/hooks";
...
const Box = ({
	...
	useFrame(() => {
		if (meshRef.current) {
			...
			const sinusoidalElapsedTime = isAnimating
				? animationClock.getElapsedTime()
				: continuousClock.getElapsedTime();

			const baseAnimFactorX =
				0.8 + Math.sin(sinusoidalElapsedTime * 0.5 + x * 0.5) * 0.2;
			const baseAnimFactorY =
				0.8 + Math.sin(sinusoidalElapsedTime * 0.5 + y * 0.5) * 0.2;
			const baseAnimFactorZ =
				0.8 + Math.sin(sinusoidalElapsedTime * 0.5 + z * 0.5) * 0.2;
	...
	return (
		<mesh position={position} ref={meshRef} userData={{ id }}>
...
const isInternal = (x: number, y: number, z: number, gridSize: number) =>
	x > 0 &&
	x < gridSize - 1 &&
	y > 0 &&
	y < gridSize - 1 &&
...
const BoxGrid = ({ performance = "high" }: { performance: "high" | "low" }) => {
	...
	const [additionallyScaledBoxIds, setAdditionallyScaledBoxIds] = useState(
		new Set<string>(),
	);
	const lastSelectionTimeRef = useRef(0);
	const selectionInterval = 3;
			...
			coordinates.flatMap((y) =>
				coordinates
					.map((z) => {
						...
						return { color, id, position, scale };
					})
		...
		if (elapsedTime - lastSelectionTimeRef.current > selectionInterval) {
			...
			if (groupRef.current && camera) {
				...
				for (let i = frontFacingBoxMeshes.length - 1; i > 0; i--) {
					...
					[frontFacingBoxMeshes[i], frontFacingBoxMeshes[j]] = [
						frontFacingBoxMeshes[j],
						frontFacingBoxMeshes[i],
					];
				}

				const selectedMeshes = frontFacingBoxMeshes.slice(
					0,
					Math.min(numToSelect, frontFacingBoxMeshes.length),
				);
				const newSelectedIds = new Set(
					selectedMeshes.map((mesh) => mesh.userData.id as string),
				);
		...
		<group ref={groupRef}>
			{boxes.map((box) => (
				<Box
					key={box.id}
					{...box}
					isAdditionallyScaled={additionallyScaledBoxIds.has(box.id)}
				/>
			))}
		</group>
	);
...
const CameraController = () => {
	...
	useFrame(() => {
		const targetPosition = isAnimating
			? new Vector3(0, 0, 2.5)
			: new Vector3(0, 0, 3);
...
export const Scene = React.memo(() => {
	...
	const [performanceLevel, setPerformanceLevel] = useState<"high" | "low">(
		"high",
	);

	const shouldVignette: boolean =
		performanceLevel === "high" &&
		["evening", "night", "dusk", "dawn", "morning"].includes(timeOfDay);
	const hasStars: boolean = ["evening", "night", "dusk", "dawn"].includes(
		timeOfDay,
	);
	const numberOfStars = performanceLevel === "high" ? 2000 : 500;

	const shouldBloom: boolean =
		performanceLevel === "high" && ["noon", "afternoon"].includes(timeOfDay);
	...
	return (
		...
		>
			...
			<color args={[colors.rgb.primary]} attach="background" />

			<BoxGrid performance={performanceLevel} />

			<EffectComposer>
				{shouldVignette ? (
					<Fragment key="vignette">
						<Vignette darkness={0.7} eskil={false} offset={0} />
					</Fragment>
				) : (
					<Fragment key="no-vignette" />
				)}
				{shouldBloom ? (
					<Fragment key="bloom">
						<Bloom
					...
					<Fragment key="no-bloom" />
				)}
				{isDark ? (
					<DepthOfField
						bokehScale={isAnimating ? 2 : 9}
						focalLength={1}
						focusDistance={0.5}
						height={720}
					/>
			...
			{hasStars && (
				<Stars
					count={hasStars ? numberOfStars : 0}
					depth={5}
					factor={timeOfDay === "night" ? 0.4 : 0.2}
					fade
					radius={1}
					saturation={timeOfDay === "night" ? 0.3 : 0.1}
				/>
			)}
