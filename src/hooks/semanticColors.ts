...
export const rgbToLab = (
	...
	const x =
		(rLinear * 0.4124564 + gLinear * 0.3575761 + bLinear * 0.1804375) / 0.95047;
	const y =
		(rLinear * 0.2126729 + gLinear * 0.7151522 + bLinear * 0.072175) / 1.0;
	const z =
		(rLinear * 0.0193339 + gLinear * 0.119192 + bLinear * 0.9503041) / 1.08883;
...
export const labToRgb = ({ l, a, b }: LabColor): [number, number, number] => {
	...
	return [rLinear, gLinear, bLinear]
		.map(toSrgb)
		.map((c) => clamp(Math.round(c * 255), 0, 255)) as [number, number, number];
};
...
export const generateSemanticColors = (
	...
	const labColor = rgbToLab(rgbColor[0], rgbColor[1], rgbColor[2], 1);

	const isDark = isDarkRgb(rgbColor);

	const primary: LabColor = {
		l: clamp(isDark ? labColor.l * 0.3 : Math.max(labColor.l, 75), 2, 95),
		a: clamp(labColor.a * (isDark ? 0.8 : 1), -100, 100),
		b: clamp(labColor.b * (isDark ? 0.8 : 1), -100, 100),
		alpha: 1,
	};

	const formatLab = (l: LabColor) =>
		`lab(${Math.round(l.l)}% ${Math.round(l.a)} ${Math.round(l.b)} / ${l.alpha})`;

	const formatRgba = (rgb: readonly [number, number, number, number]) =>
		`rgba(${Math.round(rgb[0])}, ${Math.round(rgb[1])}, ${Math.round(rgb[2])}, ${Math.round(rgb[3])})`;
	...
	const accent: LabColor = {
		l: isDark ? Math.min(85, textL + 20) : Math.max(30, textL - 25),
	...
	return {
		...
		rgb: {
			...
			secondaryAccent: formatRgba([
				...labToRgb(secondaryAccent),
				secondaryAccent.alpha * 100,
			]),
			...
			cardBackground: formatRgba([
				...labToRgb(cardBackground),
				cardBackground.alpha * 100,
			]),
		},
	};
};
